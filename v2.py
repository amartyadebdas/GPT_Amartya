...
batch_size = 64 # how many independent sequences will we procecss in parallel
...
n = int(0.9*len(data)) # First 90% will be training dataset, the rest will be validation dataset
train_data = data[:n]
val_data = data[n:]
...
class Head(nn.Module):
    ...
    def forward(self,x):
        ...
        #perform weighted aggregation on the values
        v = self.value(x)
        out = wei @ v
        return out
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head):
        #n_embd = number of dimensions | n_head = number of heads we'd like
